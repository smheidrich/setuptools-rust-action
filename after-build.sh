...
mkdir -p "$HOST_HOME_DIR_UNIX/cargo-target-dirs/$ver"
rm -rf "$HOST_HOME_DIR_UNIX/cargo-target-dirs/$ver/target"
mv "$CARGO_TARGET_DIR_UNIX" "$HOST_HOME_DIR_UNIX/cargo-target-dirs/$ver/"
...
ver=$(cat "$HOST_HOME_DIR_UNIX/outer-ver")
[ -n "$ver" ] || { echo "error loading outer version"; exit 1; }
rm -rf "$HOST_HOME_DIR_UNIX/cargo-home-dirs/$ver"
